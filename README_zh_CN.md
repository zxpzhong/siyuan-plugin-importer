...
> ✨ 时隔两年，首次更新。由于工作繁忙，插件维护一度停滞，导致部分用户无法正常使用，在此深表歉意 🙏。现在插件已经恢复维护，可以正常使用了 🚀。
> 
> 📌 请升级到思源笔记 `3.1.12` 及以上版本即可正常使用 ⚡
...
* Q1：导入工具插件怎么安装？安装之后在哪里找到它的入口？

  A1：找到 **`集市->插件->导入工具`** 下载启用即可。

  安装完成后，在顶部右侧工具栏找到 `导入工具` 图标，点击按照弹窗操作即可。

  如果启用插件后没有立即看到图标，请先确认思源笔记版本为 `3.1.12` 及以上，然后尝试重启思源笔记。
...
### Bug Fixes
* 优化样式并改进导入设置
  - 为 ImportForm 和 ImportSetting 组件添加 scoped 属性
  - 调整样式以获得更好的视觉一致性
  - 更新导入设置样式以改善用户体验
  - 对上下文菜单和其他相关样式进行微调
* 更新中文翻译，将"设置"改为"导入设置"
